...
def sequential(A, X, D, P, Q, partial_list1, J):
    ...
    for i in range(len(partial_list1) - (J - 1)):
        if i == 0:
            partial_list = generate_permutations(partial_list1, J, initial)
        else:
            result_permutations, lateness, finish = eval_perm_ct_fn_all(partial_list, A, X, D, P, Q)
...
def flow_heurDX_CT_best(A, D, X, P, Q):
    ...
    if N <= 20 : 
        ...
        last_list = sequential(A, X, D, P, Q, partial_list1, J)
        result_permutations, lateness, finish = eval_perm_ct_fn_all(last_list, A, X, D, P, Q)
...
def flow_heurDX_CT_best_Ins(A, D, X, P, Q,J):
    A = A.reshape(D.shape)  # Arrival times
    X = X.reshape(D.shape)
    if type(P) != "numpy.ndarray":
        P = P + 0 * A
...
def flow_heurDX_CT1(A, D, X, P, Q,order):
    A = A.reshape(D.shape)  # Arrival times
    X = X.reshape(D.shape)
    if type(P) != "numpy.ndarray":
        P = P + 0 * A
    ...
    partial_list1 = np.argsort(initial_orders[order])
