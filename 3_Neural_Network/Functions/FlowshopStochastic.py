...
"""
to do:
    Make a flow chart for the algorithm (done)
    Propose a Linear programming approach (MILP) to schedule the jobs while minimizing the $T^w$ and $L$.(done)
    Finish the exact 2-machine solution with MILP (uncompleteed)
    Write test program to compare the exact 2-machine solution with the stochastic algorithm.
    
Created on Fri Dec  9 01:41:58 2022
...
'''
        0. Initialization        
        1. Randomly generate initial set of  nS schedules (nS is a parameter specified at the beginning of the program).
        2. Iterate nI times:
            A. Population growth step:  while # of solutions < nA
                I. Randomly pick a solution
                II. Increase age of solution by 1
                III. Randomly pick a single machine m
                IV***. Modify due times and recalculate schedule
                V. If no change in schedule, then goto I.
                VI.If new solution is accepted:
                    i. Increase number of solutions by 1
                    ii. Add new solution
            B. Population pruning step    
                I. Remove solutions whose age exceeds the limit
                II. Sort the remaining solutions by increasing objective function
                III. Remove solutions with duplicate objective functions
                IV. Select the remaining  nS solutions with smallest objective functions
                
Step IV*** needs a more detailed description (This step is in NewStochasticSolutionFn)
Use single-machine MILP scheduling for the chosen solution and machine. The availability times are the finish times from the previous level. The due times for the jobs are the current finish times adjusted by the current job lateness. 
...
def newStochSolnFn(A,D,X,Z,Soln,m,pVec,qVec,rVec):
    N,M=X.shape
    Ytmp = 1.*Soln["Y"]
    Jtmp = 1*Soln["J"]
    ...
    Jtmp[:,m] = MILPsched1M(Atmp+Z[:,m],Dtmp,X[:,m],pVec,qVec,rVec)
...
def solnListExpandFn(initSoln,Options,nS,Age,objVec,M,A,D,X,Z,pVec,qVec,rVec,threshold):
    ...
    solDiff = list() # Did the solution change
    solDup = list() # Was the solution a duplicate
    solNew = list() # Was a new solution created
        ...
        if np.array_equal(currentSoln["J"][:,jm], newSoln["J"][:,jm]) == False:
            ...
            if newSoln["obj"] in objVec[:jL]:
                ...
                if listOfSoln[MatchSpot]["J"].all() == newSoln["J"].all():
                    ...
                    solDiff.append(solDiffTemp)
                    solDup.append(solDupTemp)
                    solNew.append(solNewTemp)
    ...
    Monitor["solDiff"] = solDiff
    Monitor["solDup"] = solDup
    Monitor["solNew"] = solNew
