...
useWeights=False #Set to True for NN Weights, False for Heuristic. Current Heuristic: flow_heurDX_CT (line 269)
P=50
Q=1
...
def newStochSolnFnNNWeight(A,D,X,Z,Soln,m,pVec,qVec,rVec):
 ...
 ##   Jtmp[:,m] = MILPsched1M(Atmp.flatten()+Z[:,m],Dtmp,X[:,m],pVec,qVec,rVec)
    if useWeights:
        ...
        Jtmp[:,m], lateness, finish=flow_heurDX_CT(A,D,X[:,m],P,Q)
